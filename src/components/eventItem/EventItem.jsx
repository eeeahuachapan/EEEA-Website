...
function EventItem({ event }) {
  ...
  const getFontSize = (title) => (title.length > 40 ? 'text-sm' : 'text-lg');

  return (
    <div onClick={onclickHandler} className="font-Montserrat cursor-pointer card card-side bg-base-100 h-fit shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px] rounded-xl flex md:w-3/4 lg:w-5/12 lg:h-[230px] ">
      ...
      <div className="card-body p-5 rounded-r-xl text-xs bg-white lg:text-sm items-stretch justify-between gap-5 w-full">
        <h1 className={`text-secondary font-bold ${getFontSize(event.title)}`}>{event.title}</h1>
        <div className='flex text-xs flex-col lg:text-sm gap-4'>
          ...
          {event.completed && (<img src="assets/done.webp" className="w-8 lg:w-9 absolute right-0 bottom-0" alt="" />)}
